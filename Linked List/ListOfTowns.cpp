...
void TownList::addTownAtStart(string name)
{
	cout << "Adding town " << name << " at start" << endl << endl;
	TownNode* current;
	if (end == NULL) //if list empty
	{
		current = new TownNode(name);  // allocate memory
		start = current;               // change start 
		end = current;				  // and end
	...
	{
		current = new TownNode(name);  //allocate memory
		current->setNext(start);       //move the start to the next node
		start = current;               //change the start with the new town
	}
